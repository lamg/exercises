...
type Statement = CreateTable | InsertInto

let keyword (s: string) = skipString s .>> spaces

let createTable = parse {
    do! keyword "CREATE"
    do! keyword "TABLE"
    return CreateTable
}

let insertInto = parse {
    do! keyword "INSERT"
    do! keyword "INTO"
    return InsertInto
}

let statement = spaces >>. (createTable <|> insertInto)
